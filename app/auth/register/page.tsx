...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      sessionStorage.setItem('otp_name', data.name)
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-violet-50 via-white to-purple-50 dark:from-violet-950/30 dark:via-background dark:to-purple-950/30 p-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <Link href="/auth/login" className="flex items-center gap-2 text-sm text-muted-foreground mb-6 hover:text-foreground transition-colors">
          <ArrowLeft className="h-4 w-4" /> Back to login
        </Link>

        <div className="text-center mb-8">
          <motion.div animate={{ rotate: [0, 10, -10, 0] }} transition={{ duration: 2, repeat: Infinity, repeatDelay: 3 }} className="text-6xl mb-3">🌟</motion.div>
          <h1 className="text-3xl font-black text-violet-700 dark:text-violet-400">Join AIVANA!</h1>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {(['child', 'parent'] as const).map((r) => (
              <label key={r} className={`relative cursor-pointer rounded-2xl border-2 p-4 text-center transition-all ${role === r ? 'border-violet-500 bg-violet-50 dark:bg-violet-900/20' : 'border-border hover:border-violet-200'}`}>
                ...
                <div className="font-semibold text-sm">{r === 'child' ? "I'm a Kid" : "I'm a Parent"}</div>
              </label>
            ))}
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input {...register('name')} type="text" placeholder={role === 'child' ? 'e.g. Aryan' : 'e.g. Rahul Sharma'}
                  className="w-full pl-10 pr-4 py-3 rounded-xl border bg-background focus:outline-none focus:ring-2 focus:ring-violet-500 text-sm transition-all" />
              </div>
